...
const Homepage = () => {
  ...
  const { data: dataFutureAppointments, refetch: refetchDataFutureApp } =
    useGetFutureAppointment(successCallBackFutureApp, userUUid, authToken)

  const { data: dataCompleteAppointment, refetch: refetchCompApp } =
    useGetCountAppointmentComplete(userUUid, authToken)
  const { data: dataCancelAppointment, refetch: refetchCancelApp } =
    useGetCountAppointmentCancel(userUUid, authToken)
  const { data: dataAllAppointment, refetch: refetchAllApp } =
    useGetCountAllAppointments(userUUid, authToken)
  useEffect(() => {
    refetch()
              ...
              <Flex w={"100%"} gap={"xs"}>
                <Stack w="100%">
                  ...
                  >
                    ...
                    <Progress
                      value={data?.data.response[0].countTime}
                      mt={5}
                      color={"#28886f"}
                    />
                    <Group position="apart" mt="md">
                      <Text fz="sm">
                        {data?.data.response[0].countTime} / 100 ore
                      </Text>
                    </Group>
                    <Text c="dimmed" ta="center" fz="sm">
                      Noi suntem mândri de tine continuă tot așa și vei fi
                      răsplătit!
                    </Text>
                  </Paper>
                ...
                >
                  ...
                  <Stack w={"100%"} align={"center"}>
                    ...
                    <Paper bg={"#7d848126"} radius="lg" w="100%">
